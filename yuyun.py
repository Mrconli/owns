...
白嫖的项目：签到白嫖游戏服务器或者稳定的虚拟主机，攒着提现也可以（2018年以来一直稳定运行的，自己可以查一查资料）
 第一步：上传本脚本到青龙（本脚本是py不是js 别搞错了）（本脚本是py不是js 别搞错了）（本脚本是py不是js 别搞错了）
 第二步：去  https://www.rainyun.cc/?ref=axiba 注册一个账号（不提现只抢游戏云和主机的话不用实名）
 想提现的话6万积分起提现...绑定支付宝提现（稳到，亲测过，也有官方用户群）。
 第三步：本脚本里第139行140行里设置一下自己注册的账号和密码！！！
积分规则及说明:本脚本是每天实现自动签到（每天300个积分），入门级游戏云或者虚拟主机2000积分一周。
这样下来一直用积分续费，想提现的话6万积分起提现...绑定支付宝提现（稳到）。
注册地址：
https://www.rainyun.cc/?ref=axiba
